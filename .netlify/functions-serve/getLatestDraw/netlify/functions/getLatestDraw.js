...
// netlify/functions/getLatestDraw.ts
var getLatestDraw_exports = {};
__export(getLatestDraw_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(getLatestDraw_exports);
...
var handler = async () => {
  try {
    ...
    const { data, error } = await supabase.from("draws").select("draw_date, contest_number, numbers").order("draw_date", { ascending: false }).limit(1).single();
    if (error) {
      return {
        statusCode: 500,
...
//# sourceMappingURL=getLatestDraw.js.map
