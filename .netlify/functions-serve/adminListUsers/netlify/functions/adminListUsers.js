...
// netlify/functions/adminListUsers.ts
var adminListUsers_exports = {};
__export(adminListUsers_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(adminListUsers_exports);
...
var handler = async (event) => {
  try {
    ...
    const { data, error } = await supabase.from("users").select("id, name, email, cpf, birth_date, created_at").order("created_at", { ascending: false });
    if (error) {
      return {
        statusCode: 500,
