...
// netlify/functions/getLastDraw.ts
var getLastDraw_exports = {};
__export(getLastDraw_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(getLastDraw_exports);
...
var handler = async () => {
  try {
    ...
    const { data, error } = await supabase.from("drawings").select("draw_date, contest_number, numbers").order("draw_date", { ascending: false }).limit(1).single();
    if (error || !data) {
      return {
        statusCode: 200,
...
//# sourceMappingURL=getLastDraw.js.map
