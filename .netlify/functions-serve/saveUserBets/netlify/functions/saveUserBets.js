...
var saveUserBets_exports = {};
__export(saveUserBets_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(saveUserBets_exports);
...
async function handler(event) {
  try {
    ...
    if (!userId || !bets || !bets.length) {
      return {
        statusCode: 400,
        body: JSON.stringify({ error: "Dados inv\xE1lidos" })
      };
    }
    const { data: draw } = await supabase.from("draws").select("id").order("draw_date", { ascending: false }).limit(1).single();
    if (!draw) {
      return {
        statusCode: 400,
...
//# sourceMappingURL=saveUserBets.js.map
