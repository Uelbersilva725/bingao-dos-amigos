...
// netlify/functions/listUserBets.ts
var listUserBets_exports = {};
__export(listUserBets_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(listUserBets_exports);
...
var handler = async (event) => {
  try {
    ...
    if (!userEmail) {
      return {
        statusCode: 401,
        body: JSON.stringify({ error: "Usu\xE1rio n\xE3o autenticado" })
      };
    }
    ...
    if (!user) {
      return {
        statusCode: 404,
        body: JSON.stringify({ error: "Usu\xE1rio n\xE3o encontrado" })
      };
    }
    const { data: bets } = await supabase.from("user_bets").select("*").eq("user_id", user.id).order("created_at", { ascending: false });
    return {
      statusCode: 200,
...
//# sourceMappingURL=listUserBets.js.map
