...
// netlify/functions/getFavoriteNumbers.ts
var getFavoriteNumbers_exports = {};
__export(getFavoriteNumbers_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(getFavoriteNumbers_exports);
...
var handler = async () => {
  try {
    ...
    const { data, error } = await supabase.from("user_games").select("numbers");
    if (error) {
      return {
        statusCode: 500,
    ...
    const favorites = Object.entries(counter).map(([number, count]) => ({
...
//# sourceMappingURL=getFavoriteNumbers.js.map
